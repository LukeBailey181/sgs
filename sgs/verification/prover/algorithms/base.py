...
from ttte.infra.prover.utils import get_datetime, load_jsonl_objects, MODEL_FORMAT
            ...
            "_extra_header": "".join(
                [
                    ...
                    for idx in np.random.choice(
                        [
                            _idx
                            for _idx, _data in enumerate(self.few_shot_dataset)
                            if _data["name"] != input_data["name"]
                        ],
                        size=self.few_shot_num,
                        replace=False,
                    )
                ]
